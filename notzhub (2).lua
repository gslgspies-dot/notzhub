...
local FileName = "NotzhubPrivate_v1.json" 
local DefaultConfig = {
    Positions = {
...
local function safeKeyCode(name, default)
...
_G.AutoInvisDuringSteal = Config.AutoInvisDuringSteal
    _G.INVISIBLE_STEAL_KEY = safeKeyCode(Config.InvisToggleKey, Enum.KeyCode.I)
...
local Theme = {
    ...
    Accent2         = Color3.fromRGB(180, 10, 10),
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    if input.KeyCode == safeKeyCode(Config.TpSettings.CarpetSpeedKey, Enum.KeyCode.Q) then
    ...
    if input.KeyCode == safeKeyCode(Config.StealSpeedKey, Enum.KeyCode.Z) then
        if SharedState.StealSpeedToggleFunc then
            SharedState.StealSpeedToggleFunc()
        end
    end

    if input.KeyCode == safeKeyCode(Config.ResetKey, Enum.KeyCode.X) then
        executeReset()
    end
    
    local ragdollKey = safeKeyCode(Config.RagdollSelfKey, nil)
...
if not IS_MOBILE then
    UserInputService.InputBegan:Connect(function(input, gp)
        ...
        local kickKC = safeKeyCode(Config.KickKey, nil)
        if kickKC and input.KeyCode == kickKC then
            kickPlayer()
        end
        local ragdollKC = safeKeyCode(Config.RagdollSelfKey, nil)
        if ragdollKC and input.KeyCode == ragdollKC then
            task.spawn(function()
                if _G.runAdminCommand then
                    local onCD = _G.isOnCooldown and _G.isOnCooldown("ragdoll")
                    if not onCD then
                        if _G.runAdminCommand(LocalPlayer, "ragdoll") then
        ...
        local proxKC = safeKeyCode(Config.ProximityAPKeybind, nil)
        if proxKC and input.KeyCode == proxKC then
            ProximityAPActive = not ProximityAPActive
            if SharedState.ProximityAPButton then
                pcall(function()
                    SharedState.ProximityAPButton.BackgroundColor3 = ProximityAPActive and Theme.Accent1 or Color3.fromRGB(35, 37, 43)
        ...
        local joinerKC = safeKeyCode(Config.JobJoinerKey, nil)
        if joinerKC and input.KeyCode == joinerKC then
...
task.spawn(function()
    ...
    if privateBuild then
        ...
        author.Text = "discord.gg/notzhub"
    end
